...
# need to scale for SVM otherwise features with larger ranges dominate
...
class LinearSVM:
    ...
    def fit(self, X, y):
        n, d = X.shape
        # labels need to be -1/+1 for hinge loss
        y_ = np.where(y == 0, -1, 1)
...
svm = LinearSVM(lr=0.0001, lam=0.01, n_iters=500)
svm.fit(X_train, y_train)

preds = svm.predict(X_val)
print(f"\nAccuracy: {svm.score(X_val, y_val):.4f}")
print(classification_report(y_val, preds, target_names=["Not Pulsar", "Pulsar"]))
